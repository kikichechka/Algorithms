...
class Tree:
    ...
    def add(self, value: int):
        ...
        if self.root is not None:
            result = self.add_data(self.root, value)
    ...
    def balance(self, node) -> Node:
        ...
        while making_balance:

            making_balance = False
    ...
    def add_data(self, node: Node, value: int):
        if (node.value == value):
            return False
        ...
        if node.value > value:
            if node.left is not None:
                result = self.add_data(node.left, value)
            ...
            if (node.right is not None):
                result = self.add_data(node.right, value)
